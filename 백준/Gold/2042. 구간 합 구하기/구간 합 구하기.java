...
public class Main {
	public static void main(String[] args) {
		...
		for(int i=0; i < M + K; i++) {
			...
			c = sc.nextLong();
			//변경 시
			if(a == 1) tree.updateDirect(1, 1, N, b, c);
			//구간합 구하기
			else sb.append(tree.sum(1, 1, N, b, c) + "\n");
		}
	...
	static class SegmentTree {
		...
		*/
		...
		//배열에 특정 인덱스의 값이 변경될 경우 세그먼트 트리의 노드 값 변경 (차이 값을 더하는 방법)
		void updateDiff(int node, int start, int end, long idx, long diff) {
			...
			if(idx < start || end < idx) return;
				...
				//노드가 리프 노드가 아닌 경우
				if(start != end) {
					//리프 노드까지 계속 자식 노드를 탐색
					updateDiff(node * 2, start, (start + end) / 2, idx, diff); //왼쪽 자식 노드
					updateDiff(node * 2 + 1, (start + end) / 2 + 1, end, idx, diff); //왼쪽 자식 노드
				}
			}
		}
